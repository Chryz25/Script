    ...
    for ext in $IMAGE_EXTS; do
        while IFS= read -r -d $'' image_file; do
            if [[ -f "$image_file" ]]; then
                ...
                # Using ImageMagick to compress images (quality 75)
                convert "$image_file" -quality 75 "$output_file" &>/dev/null

                if [[ $? -eq 0 ]]; then
                    echo -e "  ${GREEN}Compressed: ${filename} -> $(basename "$output_file") (Quality 75)${RESET}"
                    # If the original was not a jpg, we might want to remove the original and keep the jpg
                    # But to keep it simple and consistent with the video logic (replacing original):
